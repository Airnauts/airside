...
function deps(over = {}) {
  ...
  const repo = {
    ...
    upsertExternalLink: vi
      .fn()
      .mockImplementation((_s, _id, link) => Promise.resolve({ ...thread, externalLinks: [link] })),
  }
  const link = {
    provider: 'jira',
    externalId: '1',
...
describe('runThreadAction', () => {
  ...
  it('404 when the action is not registered', async () => {
    await expect(runThreadAction(input('nope.x') as never, deps() as never)).rejects.toBeInstanceOf(
      NotFoundError,
    )
  })
  ...
  it('404 when the thread does not exist', async () => {
    ...
    await expect(
      runThreadAction(input('jira.createIssue') as never, d as never),
